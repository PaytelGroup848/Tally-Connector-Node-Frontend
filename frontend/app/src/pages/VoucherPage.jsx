...
function VoucherPage({ type = 'Quotation' }) {
  ...
  const addRow = () => setRows((currentRows) => [...currentRows, { id: Date.now() }])
  ...
  return (
    <div className="min-h-[calc(100vh-60px)] bg-[#eef3f8] p-5 text-slate-900">
      <div className="mx-auto max-w-[1280px] overflow-hidden rounded-lg border border-slate-200 bg-white shadow-[0_10px_30px_rgba(24,33,43,0.05)]">
        ...
        <div className="bg-[#f5f7f4] p-5">
          ...
          <div className="mt-5 overflow-hidden rounded-lg border border-slate-200 bg-white">
            <div className="grid min-w-[1000px] grid-cols-[1.15fr_0.45fr_0.45fr_0.5fr_0.5fr_0.9fr_0.8fr_1.3fr_0.7fr_0.5fr_0.35fr] gap-2 bg-slate-100 p-2 text-[12px] font-semibold uppercase tracking-wide text-slate-600">
              ...
              <b>Amount</b>
              <b>Tax Incl.</b>
              <button type="button" onClick={addRow} className="justify-self-end rounded-md bg-[#dff5e5] px-2 py-1 text-[10px] font-bold uppercase text-green-700">+</button>
            </div>

            {rows.map((row) => (
              <div key={row.id} className="grid min-w-[1000px] grid-cols-[1.15fr_0.45fr_0.45fr_0.5fr_0.5fr_0.9fr_0.8fr_1.3fr_0.7fr_0.5fr_0.35fr] gap-2 border-t border-slate-200 bg-white p-2">
            ...
            <div className="flex-1 rounded-md border border-slate-200 bg-white">
              {['Narration', 'Advanced Settings'].map((section) => (
                <button
                  ...
                  className="flex w-full items-center justify-between border-b border-slate-200 px-4 py-3 text-left text-[14px] font-medium text-slate-700 last:border-b-0"
                >
                  <span>{section}</span>
        ...
        <div className="flex justify-end bg-[#f5f7f4] px-5 pb-5 pt-3">
          {isSales && (
            <button type="button" className="mr-3 rounded-lg border border-slate-300 bg-white px-5 py-3 text-sm font-semibold text-slate-800 shadow-[0_10px_20px_rgba(24,33,43,0.08)]">
              Create eWay
            </button>
          )}
