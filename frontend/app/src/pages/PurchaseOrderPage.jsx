  ...
  return (
    <div className="min-h-[calc(100vh-60px)] bg-[#eef3f8] p-5 text-slate-900">
      <div className="mx-auto max-w-[1280px] overflow-hidden rounded-lg border border-slate-200 bg-white shadow-[0_10px_30px_rgba(24,33,43,0.05)]">
        <div className="bg-[#63c45d] px-5 py-4 text-[17px] font-bold text-white">Create Purchase Order Voucher</div>
        ...
        <div className="bg-[#f5f7f4] p-5">
          ...
          <div className="mt-5 overflow-hidden rounded-lg border border-slate-200 bg-white">
            ...
            {rows.map((row) => (
              <div key={row.id} className="grid min-w-[1000px] grid-cols-[1.2fr_0.45fr_0.45fr_0.5fr_0.5fr_0.9fr_0.8fr_1.3fr_0.7fr_0.5fr_0.35fr] gap-2 border-t border-slate-200 bg-white p-2">
          ...
          <div className="mt-5 flex flex-col gap-4 xl:flex-row">
            <div className="flex-1 rounded-md border border-slate-200 bg-white">
              <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3 text-[14px] font-medium text-slate-700">
                <span>Narration</span>
                <span>›</span>
              </div>

              <button type="button" onClick={() => setAdvancedOpen((value) => !value)} className="flex w-full items-center justify-between border-b border-slate-200 px-4 py-3 text-left text-[14px] font-medium text-slate-700">
              ...
              {advancedOpen && (
                <div className="bg-[#f7f9f8] p-4">
                  <div className="mb-4 flex flex-wrap gap-2">
                    {['Supplier’s Details', 'Consignee Details', 'Dispatch Details', 'Order Details'].map((tab, index) => (
                      <button key={tab} type="button" className={index === 0 ? 'rounded-md border border-slate-900 bg-white px-3 py-2 text-[11px] font-semibold text-slate-800' : 'rounded-md border border-slate-200 bg-slate-100 px-3 py-2 text-[11px] font-medium text-slate-600'}>
                        {tab}
                      </button>
                    ))}
                  </div>
                  ...
                  <div className="grid gap-3 md:grid-cols-2">
                    {detailFields.map(([label, placeholder]) => (
                      <label key={label} className="flex flex-col gap-1 text-[12px] font-medium text-slate-700">
                        <span>{label}</span>
                        <input placeholder={placeholder} className="min-h-10 rounded-md border border-slate-300 bg-white px-3 text-sm text-slate-700 outline-none placeholder:text-slate-400 focus:border-green-600 focus:ring-2 focus:ring-green-100" />
                      </label>
                    ))}
                    <label className="md:col-span-2 flex flex-col gap-1 text-[12px] font-medium text-slate-700">
                      <span>Address</span>
                      <textarea placeholder="Address" className="min-h-[72px] rounded-md border border-slate-300 bg-white px-3 py-2 text-sm text-slate-700 outline-none placeholder:text-slate-400 focus:border-green-600 focus:ring-2 focus:ring-green-100" />
                    </label>
                  </div>
                </div>
              )}
