...
const normalizeFieldName = (field) =>
  String(field).replace(/[_-]/g, "").toLowerCase();
...
const isHiddenSalesField = (field) =>
  normalizedHiddenSalesFields.has(normalizeFieldName(field));
...
function ReportTable({
  ...
  const currentPage = currentPageOverride ?? internalCurrentPage;

  const pageSize = pageSizeOverride ?? internalPageSize;

  const setReportPage = (nextPage) => {
    if (onPageChange) {
  ...
  const totalItems = totalItemsOverride ?? rows.length;

  const totalPages = Math.max(1, Math.ceil(totalItems / pageSize));

  useEffect(() => {
    setInternalCurrentPage((page) => Math.min(page, totalPages));
  }, [totalPages]);
      ...
      >
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            onClick={() => setReportPage(Math.max(1, currentPage - 1))}
            className="
              flex
              h-8
              w-8
          ...
          <button
            ...
            onClick={() => setReportPage(Math.min(totalPages, currentPage + 1))}
            className="
              flex
              h-8
              w-8
...
function ReceivablesReport({ config, query, setQuery, companyId }) {
  ...
  const fetchRecords = isSupplierReport ? fetchSuppliers : fetchCustomers;

  const extractRecords = isSupplierReport ? extractSuppliers : extractCustomers;

  const entityLabel = isSupplierReport ? "sundry creditors" : "sundry debtors";
  ...
  useEffect(() => {
    if (!accessToken) return; // guard add karo
  ...
  useEffect(() => {
    ...
    fetchRecords({
      ...
      .then((response) => {
        ...
        const customers = extractRecords(response);
        ...
        setRows(
          customers.map((customer, index) => {
            ...
            const creditDays =
              customer?.creditDays ?? customer?.credit_days ?? "-";
        ...
        const responsePages = Number(
          pagination.totalPages ?? pagination.pages ?? pagination.lastPage,
        );
      ...
      .catch((error) => {
        ...
        setErrorMessage(error.message || `Unable to load ${entityLabel}`);
      })
      .finally(() => {
        if (isMounted) {
          setIsLoading(false);
  ...
  const customerColumns = [
    ...
    },
    {
      key: "gstin",
      label: "GSTIN",
      aliases: ["gstin", "gstIn", "gstNumber", "gst_number"],
    },
    {
      key: "openingBalance",
    ...
    },
    {
      key: "createdAt",
      label: "Created At",
      aliases: ["createdAt", "created_at", "createdDate", "created_date"],
    },
    {
      key: "updatedAt",
      label: "Updated At",
      aliases: ["updatedAt", "updated_at", "updatedDate", "updated_date"],
    },
    {
      key: "phone",
      label: "Phone",
  ...
  const getCustomerValue = (customer, column) => {
    const matchingKey = column.aliases.find(
      (alias) => customer?.[alias] !== undefined,
    );

    if (matchingKey) {
      return customer[matchingKey];
    }

    const normalizedAliases = column.aliases.map((alias) =>
      alias.replace(/[_-]+/g, "").toLowerCase(),
    );

    const caseInsensitiveKey = Object.keys(customer || {}).find((key) =>
      normalizedAliases.includes(key.replace(/[_-]+/g, "").toLowerCase()),
    );

    return caseInsensitiveKey ? customer[caseInsensitiveKey] : undefined;
  };

  const formatCustomerValue = (value, columnKey) => {
    if (value === null || value === undefined || value === "") {
      return columnKey === "gstin" ? "-" : "-";
    }
  ...
  const formattedOutstanding =
    totalOutstanding === null && displayedOutstanding === 0
      ? "-"
  ...
  const handleReminder = (customer) => {
    ...
    console.log("this is my phone ", customer);
    ...
    // template state already fetched via GET /reminder-template on page load
    const message = reminderTemplate
      .replace("{customerName}", customer?.name || "Customer")
      .replace("{outstandingAmount}", customer?.closingBalance)
  ...
  const handleEmailReminder = async (customer) => {
    if (!customer?.email) {
      ...
      const res = await fetch(
        `https://connector.cloudata.in/api/companies/${companyId}/reminders/email`,
        {
          ...
          body: JSON.stringify({
            ...
            partyType: isSupplierReport ? "SUPPLIER" : "CUSTOMER",
          }),
        },
      );
      const json = await res.json();
      if (!res.ok) throw new Error(json.message || "Failed to send reminder");

      setSentMessage(`Reminder emailed to ${customer.name}`);
  ...
  const handleBulkReminder = () => {
    setSentMessage("Bulk reminders queued successfully");
  ...
  const displayRows = customerRecords.map((customer, index) => ({
    customer,
    ...
    action: (
      ...
      >
        ...
        <button
          type="button"
          onClick={() => handleReminder(customer)}
          className=" text-lg
                flex
                items-center
                justify-center
                ...
                {displayRows.map((row) => (
                  ...
                  >
                    ...
                    <td className="whitespace-nowrap px-5 py-3">
                      <div className="flex justify-center">{row.action}</div>
                    </td>
                  </tr>
                ))}
              </tbody>
    ...
    <div className="report-page min-h-[calc(100vh-60px)] bg-[#eef3f8] text-slate-900">
      <div className="flex min-h-[64px] items-center gap-4 border-b border-slate-200 bg-white px-4">
        ...
        <div className="text-[11px] leading-4">
          <b className="block">{label.trim()}</b>

          <strong className="block text-[18px]">₹{amount}</strong>
        </div>
      ...
      <section className="mx-5 mt-2.5 overflow-hidden rounded-lg bg-white shadow">
        <div className="flex flex-wrap items-center gap-4 px-5 py-3">
          ...
          <span className="ml-auto text-xs">Rows are paginated below</span>
        </div>

        <ReportTable columns={config.columns} rows={filteredRows} />
      </section>
    </div>
  );
}
...
function SalesReport({ companyId }) {
  const accessToken = useAuthStore((state) => state.accessToken);
  ...
  useEffect(() => {
    ...
    fetchCompanySales(accessToken, companyId, {
      q: query,
      from: startDate,
      to: endDate,
      page: currentPage,
      ...
      .then((response) => {
        ...
        const sales = extractSales(response);

        const getRawValue = (entry, aliases) => {
          ...
          const key = Object.keys(candidate).find((name) =>
            normalizedAliases.includes(
        ...
        const totalFromRows = sales.reduce(
          (sum, sale) =>
            sum +
            amount(
              getRawValue(sale, [
        ...
        const pagination = extractSalesPagination(response);
        ...
        setTotalItems(
          Number.isFinite(responseTotal) ? responseTotal : sales.length,
        );

        setTotalAmount(
          Number(
            pagination.totalAmount ?? pagination.total_amount ?? totalFromRows,
          ) || 0,
        );
      })
      .catch((error) => {
  ...
  const columnLabel = (column) =>
    column.replace(/[_-]+/g, " ").replace(/([a-z])([A-Z])/g, "$1 $2");

  const salesTotalPages = Math.max(1, Math.ceil(totalItems / pageSize));
    ...
    <div className="min-h-[calc(100vh-60px)] bg-[#eef3f8]">
      ...
      <div className="bg-white px-5 py-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <span className="block text-xs text-slate-600">Total Sales:</span>
      ...
      <section className="mx-3 mt-2 overflow-hidden rounded-[6px] border border-white bg-white shadow-[0_3px_15px_rgba(24,33,43,0.06)]">
              ...
              <tbody className="divide-y divide-slate-100">
                {rows.length > 0 ? (
                  rows.map((row, rowIndex) => (
                    ...
                    >
                      {salesColumns.map((column) => {
                        const value = displayValue(row?.[column], column);
                  ...
                  <tr>
                    <td
                      colSpan={Math.max(salesColumns.length, 1)}
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 border-t border-slate-100 px-6 py-4">
          ...
          <button
            type="button"
            disabled={isLoading || currentPage === salesTotalPages}
            onClick={() =>
              setCurrentPage(Math.min(salesTotalPages, currentPage + 1))
            }
            className="
              flex
              h-9
  ...
  useEffect(() => {
    ...
    fetchCompanyCash(accessToken, companyId, {
      ...
      .then((response) => {
        ...
        setRows(cashRows);
        setTotalItems(
          Number.isFinite(responseTotal) ? responseTotal : cashRows.length,
        );
      })
      .catch((error) => {
        if (!mounted) return;
  ...
  useEffect(() => {
    ...
    fetchCompanyBank(accessToken, companyId, {
      ...
      .then((response) => {
        ...
        setRows(bankRows);
        setTotalItems(
          Number.isFinite(responseTotal) ? responseTotal : bankRows.length,
        );
      })
      .catch((error) => {
        if (!mounted) return;
  ...
  useEffect(() => {
    ...
    fetchRecords(accessToken, effectiveCompanyId, {
      ...
      .then((response) => {
        ...
        setRows(creditNotes);
        setTotalItems(
          Number.isFinite(responseTotal) ? responseTotal : creditNotes.length,
        );
      })
      .catch((error) => {
        if (!mounted) return;
          ...
          {Array.from({ length: totalPages }, (_, index) => index + 1)
            ...
            .map((page) => (
              <button
                ...
                disabled={isLoading}
                onClick={() => setCurrentPage(page)}
                className={`h-9 min-w-9 rounded-lg border px-2 text-sm font-medium ${currentPage === page ? "border-emerald-600 bg-emerald-600 text-white" : "border-slate-200 text-slate-700 hover:border-emerald-300 hover:bg-emerald-50"} disabled:opacity-50`}
              >
                {page}
              </button>
            ))}
...
function ReportListPage({ path, companyId }) {
  ...
  if (path === "/sales") {
    return <SalesReport companyId={companyId} />;
  }

  if (path === "/cash-bank/cash") {
    return <CashReport companyId={companyId} />;
  }

  if (path === "/cash-bank/bank") {
    return <BankReport companyId={companyId} />;
  }

  if (path === "/creditnote") {
    return <CreditNoteReport companyId={companyId} />;
  }

  if (path === "/purchaseorder") {
    return (
      <CreditNoteReport companyId={companyId} reportType="purchaseorder" />
    );
  }

  if (path === "/payments") {
    return <CreditNoteReport companyId={companyId} reportType="payment" />;
  }

  if (path === "/debitnote") {
    return <CreditNoteReport companyId={companyId} reportType="debitnote" />;
  }

  if (path === "/receipt") {
    return <CreditNoteReport companyId={companyId} reportType="receipt" />;
  }

  if (path === "/receiptnote") {
    return <CreditNoteReport companyId={companyId} reportType="receiptnote" />;
  }

  if (path === "/salesorder") {
    return <CreditNoteReport companyId={companyId} reportType="salesorder" />;
  }

  if (path === "/deliverynote") {
    return <CreditNoteReport companyId={companyId} reportType="deliverynote" />;
  }
    ...
    <div className="min-h-[calc(100vh-60px)] bg-[#eef3f8]">
      <div className="bg-white px-5 py-4">
        <div className="flex items-center justify-between">
          <div>
            <span className="block text-xs text-slate-600">{config.total}</span>
